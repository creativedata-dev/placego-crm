...
export function WhatsAppConnector({ companyId, instanceName, channel }: Props) {
  ...
  if (loading) return <div className="flex items-center gap-2 text-sm text-muted-foreground py-4"><Loader2 className="h-4 w-4 animate-spin" /> Verificando...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {status === "connected" && <span className="text-xs text-green-600 font-medium flex items-center gap-1"><CheckCircle className="h-3.5 w-3.5" /> Conectado</span>}
        {status === "connecting" && <span className="text-xs text-yellow-600 font-medium flex items-center gap-1"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Aguardando QR</span>}
        {status === "disconnected" && <span className="text-xs text-muted-foreground">Desconectado</span>}
      </div>
      ...
      {status === "connecting" && qrCode && (
        <div className="flex flex-col items-center gap-3 p-4 border-2 border-dashed rounded-xl">
          ...
          <p className="text-xs text-muted-foreground text-center">WhatsApp → Configurações → Aparelhos conectados</p>
          <Button variant="outline" size="sm" onClick={checkStatus}><RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Atualizar</Button>
        </div>
      )}
      ...
      <div className="flex gap-2">
        ...
        {status === "connected" && (
          <Button size="sm" onClick={handleSaveWelcome} disabled={saving}>
        ...
        {status === "connecting" && !qrCode && (
          <Button size="sm" variant="outline" onClick={checkStatus}><RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Buscar QR</Button>
        )}
      </div>
    </div>
  );
